...
## 11. Repository Customization (training-currency-converter)

These rules tailor the generic Next.js guidance to this repository.

- **Architecture defaults:**
  - Use `useExchangeRates` for rate acquisition, loading/error handling, and fallback behavior.
  - Use `useConverter` for conversion state transitions and conversion logic orchestration.
  - Keep conversion state URL-driven where applicable, aligned with `useSearchParams` patterns.
- **Component composition:**
  - Prefer small, focused components under `components/`.
  - Keep complex logic in hooks/utilities, not presentational components.
- **API layer behavior:**
  - Preserve multi-source fallback behavior in rates fetching.
  - Keep caching behavior consistent with the one-hour expectation unless a task explicitly changes it.
  - Never remove offline/local fallback behavior without explicit approval.
- **Testing conventions:**
  - Co-locate tests with source files (for example, `Component.tsx` + `Component.test.tsx`).
  - For hook tests that mock network behavior, prefer the existing Jest + MSW-compatible patterns in the repo.
  - Every bug fix should add or update at least one regression test when practical.
- **Validation defaults after changes:**
  - `npm run lint`
  - `npm run test -- --runInBand`

## 12. Practical Guardrails for Agents

- Prefer minimal patches that solve root cause over broad refactors.
- Do not add new dependencies when existing utilities/patterns already solve the problem.
- Avoid creating example/demo files unless explicitly requested.
- If uncertain about a behavior change, keep existing user-visible behavior and ask for clarification.